...
def fire(event):                               #Функция вычисления площади и pi
    ...
    canv.create_rectangle(15, 15, R*2+15, R*2+15, outline="black", fill="white")     #Создание квадрата 
    canv.create_oval(15, 15, R*2+15, R*2+15,  outline="black", fill="white")         #Создание  окружности 
    ...
    for i in range(N):                        #Цикл генерации точек
                   ...
                   x_0=15+R                  # вычисление точки середины окружности
                   y_0=15+R
               
                   if (x-x_0)**2+(y-y_0)**2<R**2:         #Проверяем попали в окружность
                       M +=1
                       canv.create_oval(x, y, x+2, y+2, outline="orange", fill="orange") #Отображаем созданную точку
                   else:
                       canv.create_oval(x, y, x+2, y+2, outline="green", fill="green") #Отображаем созданную точку
              
                        
                
    otv_s["text"] = "{:2.2f}".format(4*(R**2)*M/N)      #Подсчет площади окружности
    otv_pi["text"] = "{:2.5f}".format(4*M/N)             #Подсчет pi
